...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        //查出是否有此用户
        logger.info("查出是否有此用户");  
        UUser hasUser = null;
		try {
			hasUser = adminService.findByName(token.getUsername());
		} catch (Exception e1) {
...
//        String md5Pwd = new Md5Hash("123", "lucare",2).toString();
        if (hasUser != null) {
        	...
        	List<URole> rlist = new ArrayList<>();
        	List<UPermission> plist = new ArrayList<>();
			try {
				rlist = adminService.findRoleByUid(hasUser.getId());//获取用户角色
				plist = adminService.findPermissionByUid(hasUser.getId());//获取用户权限
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
